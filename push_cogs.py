...
def push_cog_files():
    """Push essential cog files to GitHub"""
    if not TOKEN:
        print('GitHub token is not set. Please set the GITHUB_TOKEN environment variable.')
        return False
    
    print(f"Preparing to push {len(COG_FILES)} cog files")
    ...
    print(f'\n{successful} out of {len(COG_FILES)} cog files pushed successfully!')
    return successful > 0

if __name__ == '__main__':
    print('Pushing cog files to GitHub repository...')
    success = push_cog_files()
    
    if success:
        print(f'\nCog files pushed to https://github.com/{REPO_OWNER}/{REPO_NAME}')
    else:
        print('\nFailed to push cog files to GitHub')
